@extends('app', ['title' => $product->name.' Edit'])
@push('stylesheet')
...
@section('content')
    ...
    @endif
    <!-- start: page -->
    <form class="betuls-product-form action-buttons-fixed" action="{{ route('admin.products.update', ['slug' => $product->slug]) }}" method="post">
        @csrf
        <div class="row mt-2">
            <div class="col">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-3-5 col-xl-4-5">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-lg-7 col-xl-6">
                                        <textarea class="form-control form-control-modern" name="detail" id="detail" rows="6" required>{{ $product->description }}</textarea>
                                    </div>
                                </div>
                                <div class="form-group row">
                                        ...
                                        <div id="categoryTree">
                                            <ul>
                                                @foreach($categories as $category)
                                                    <li id="{{ $category->id }}" data-jstree='{"selected":true}'>
...
{{--                                    {{ print_r($product->images) }}--}}
                                    @foreach($product->images as $image)
                                        <div class="col-3">
                                            <img width="100%" src="{{ asset(Storage::url($image->image)) }}" alt="">
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                ...
                <section class="card card-modern card-big-info">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-lg-2-5 col-xl-1-5">
                                <i class="card-big-info-icon bx bx-camera"></i>
                                <h2 class="card-big-info-title">Product Reviews</h2>
                                <p class="card-big-info-desc">Reviews from customer for this product</p>
                            </div>
                            <div class="col-lg-3-5 col-xl-4-5">
                                <div class="form-group row align-items-center">
                                    {{ $product->getAverageRatingAttribute() }}<br>
                                    @foreach($product->variantReviews as $review)
                                        {{ $review->created_at }} - {{ $review->comment }}<br>
                                    @endforeach
                                </div>
                            </div>
                        </div>
                ...
                <section class="card card-modern card-big-info">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-lg-2-5 col-xl-1-5">
                                <i class="card-big-info-icon bx bx-camera"></i>
                                <h2 class="card-big-info-title">Product Variants</h2>
                                <p class="card-big-info-desc">Added product variants listed.</p>
                            </div>
                            <div class="col-lg-3-5 col-xl-4-5">
                                <div class="form-group row align-items-center">
                                    <div class="col">
                                        <table class="table table-responsive-md mb-0">
                                            ...
                                            <tr>
                                                <th>ID</th>
                                                @foreach($product->variantOptions as $optionName => $option)
                                                    <th>{{ $optionName }}</th>
                                                @endforeach
                                                <th>Price</th>
                                                <th>Stock</th>
                                            ...
                                            @foreach($product->variants as $variant)
                                                <tr>
                                                    <td>{{ $variant->id }}</td>
                                                    @foreach($product->variantOptions as $optionName => $option)
                                                        <td>{{ $variant->getOptionValue($optionName) }}</td>
                                                    @endforeach
                                                    <td>{{ $variant->price }}</td>
                                                    <td>{{ $variant->stock }}</td>
                                                </tr>
